...
def banner():
    ...
    print("\033[1;36m          KURANOMI - WiFi Bypass Super Stable Gaming Edition 1 \033[0m")
...
async def auto_loop_bypass(session_url, mac_address, voucher, gateway_ip):
    ...
    print("\n" + "="*56)
    print("  🔄 Stable Bypass Loop Started")
    print(f"  ⏱️  Interval: {loop_interval} seconds (4 minutes)")
    ...
    while auto_loop_running:
        ...
        if success:
            ...
            print("\n  ⚠️ Bypass failed, retrying in 4 minutes...")
        
        # Wait for 330 seconds
        print(f"\n  ⏳ Waiting 240 seconds before next loop...")
...
def start_bypass():
    ...
    # Use fixed URL - no need to ask for it
    ...
    print(f"\033[1;34m[ Fixed URL ကို အလိုအလျောက်သုံးပါမည် ]\033[0m")
    print(f"\033[1;34m[ URL ]: {FIXED_URL[:80]}...\033[0m\n")
    ...
    print("\n\033[1;33m[*] Internet Bypass စတင်နေပါပြီ (Bypass Fail ပြလည်း Ping ပေါ်ရင်သုံးလို့ရပါတယ်)\033[0m")
